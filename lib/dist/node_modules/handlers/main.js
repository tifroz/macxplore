...
main = function(app, cacheConfig) {
  ...
  app.get("/report/sampledoc/:_id", cacheMiddleWare, function(req, res) {
    var colname, dbname, limit, queryLimit, report;
    dbname = null;
    colname = null;
    report = new Report({
      _id: req.params._id
    ...
    return Seq().seq(function() {
      ...
      return cursor.limit(limit).stream().pipe(cursor2JsonArray()).pipe(res);
    }).catch(function(boo) {
      return handleError(res, boo);
    });
  });
  ...
  return app.get("/refactorreports", function(req, res) {
    return res.send("Refactoring functionality removed");
  });
};
